...
{
    [CustomEditor(typeof(ColorTargetRawImage))]
    [CanEditMultipleObjects]
    public class ColorTargetRawImageEditor : BaseEditor
    {
        protected override ColorName ComponentColorName { get { return DGUI.Colors.ThemesColorName; } }

        private ColorTargetRawImage m_target;

        private ColorTargetRawImage Target
        {
            get
            {
                if (m_target != null) return m_target;
                m_target = (ColorTargetRawImage) target;
                return m_target;
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            DrawHeader(Styles.GetStyle(Styles.StyleName.ComponentHeaderColorTargetRawImage), MenuUtils.ColorTargetRawImage_Manual, MenuUtils.ColorTargetRawImage_YouTube);
            GUILayout.Space(DGUI.Properties.Space(2));
            DGUI.Property.Draw(m_image, UILabels.RawImage, HasReference ? ComponentColorName : ColorName.Red);
            GUILayout.Space(DGUI.Properties.Space());
            ThemeTargetEditorUtils.DrawOverrideAlpha(m_overrideAlpha, m_alpha, Target.Image == null ? 1 : Target.Image.color.a, ComponentColorName, InitialGUIColor);
