...
{
    [CustomEditor(typeof(TextureTargetRawImage))]
    [CanEditMultipleObjects]
    public class TextureTargetRawImageEditor : BaseEditor
    {
         protected override ColorName ComponentColorName { get { return DGUI.Colors.ThemesColorName; } }

        private TextureTargetRawImage m_target;

        private TextureTargetRawImage Target
        {
            get
            {
                if (m_target != null) return m_target;
                m_target = (TextureTargetRawImage) target;
                return m_target;
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            DrawHeader(Styles.GetStyle(Styles.StyleName.ComponentHeaderTextureTargetRawImage), MenuUtils.TextureTargetRawImage_Manual, MenuUtils.TextureTargetRawImage_YouTube);
            GUILayout.Space(DGUI.Properties.Space(2));
            DGUI.Property.Draw(m_image, UILabels.RawImage, HasReference ? ComponentColorName : ColorName.Red);
            ...
            GUILayout.Space(DGUI.Properties.Space(2));
            int propertyIndex = m_theme.GetTexturePropertyIndex(Target.PropertyId);
            if (Target.PropertyId == Guid.Empty || propertyIndex == -1) ThemeTargetEditorUtils.DrawLabelNoPropertyFound();
            else ThemeTargetEditorUtils.DrawTextureProperties(m_theme, propertyIndex, serializedObject, targets, Target, ComponentColorName, InitialGUIColor);
            GUILayout.Space(DGUI.Properties.Space(4));
        ...
        private void UpdateIds()
        {
            ...
            if (!m_theme.ContainsTextureProperty(Target.PropertyId))
                Target.PropertyId = m_theme.TextureLabels.Count > 0
                                        ? m_theme.TextureLabels[0].Id
                                        : Guid.Empty;
        }
