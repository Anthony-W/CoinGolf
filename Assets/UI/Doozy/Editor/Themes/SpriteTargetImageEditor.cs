...
{
    [CustomEditor(typeof(SpriteTargetImage))]
    [CanEditMultipleObjects]
    public class SpriteTargetImageEditor : BaseEditor
    {
        protected override ColorName ComponentColorName { get { return DGUI.Colors.ThemesColorName; } }

        private SpriteTargetImage m_target;

        private SpriteTargetImage Target
        {
            get
            {
                if (m_target != null) return m_target;
                m_target = (SpriteTargetImage) target;
                return m_target;
            }
        }
        ...
        private void UpdateLists() { ThemesNames = ThemesDatabase.GetThemesNames(Database); }
    }
}