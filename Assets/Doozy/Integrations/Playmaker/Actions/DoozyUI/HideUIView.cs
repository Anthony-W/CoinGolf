...
namespace Doozy.Integrations.Playmaker
{
    ...
    [Tooltip("Hide an UIView")]
    public class HideUIView : FsmStateAction
    {
        ...
        [UIHint(UIHint.FsmBool)]
        [Tooltip("Determines if the HIDE animation should happen instantly (in zero seconds)")]
        ...
        {
            if (DebugMode.Value) DDebug.Log("Playmaker - State Name [" + State.Name + "] - Hide UIView");
        ...
        {
            yield return null;
            UIView.HideView(ViewCategory.Value, ViewName.Value, InstantAction.Value);
        }
    }
}
